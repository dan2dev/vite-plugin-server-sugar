...
function bunCompileArtifacts(dir: string): Set<string> {
  return new Set(
    readdirSync(dir).filter((name) => bunCompileArtifactPattern.test(name)),
  );
}
...
export function writeServerSource(
  source: string,
  serverOutDir: string,
): string {
...
async function bundleModuleSource(
  ...
  try {
    ...
    const { output } = await bundle.generate({
      ...
      codeSplitting: false,
      comments: false,
      banner: "// Auto-generated by vite-plugin-server-sugar - do not edit",
    });
