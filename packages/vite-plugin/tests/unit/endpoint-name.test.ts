...
function processAndGetEndpoints(
  ...
  filePath: string,
  root: string,
): { actionEndpoints: string[]; wsEndpoints: string[] } {
  ...
  const actionEndpoints = [...registry.getEndpointsForFile(filePath)];
  const wsEndpoints = [...wsRegistry.getEndpointsForFile(filePath)];

  return { actionEndpoints, wsEndpoints };
}
...
describe('endpoint name generation', () => {
  describe('src/ prefix stripping from file paths', () => {
    it('strips the src/ prefix from the file path', () => {
      const code = `const getTodos = $action(() => [])`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
        '/project/src/todos.ts',
        '/project',
      );

      expect(actionEndpoints[0]).toBe('todos/get-todos');
    });
    ...
    it('does not strip src/ when not at the start of the relative path', () => {
      ...
      expect(actionEndpoints[0]).toBe('lib/src/todos/get-todos');
    });

    it('keeps the path as-is when there is no src/ prefix', () => {
      const code = `const getUsers = $action(() => [])`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('api/users/get-users');
    });

    it('strips file extension (.ts, .tsx, .js, .jsx) from the path', () => {
      const code = `const handler = $action(() => null)`;
      ...
      expect(tsx.actionEndpoints[0]).toBe('page/handler');
      ...
      expect(js.actionEndpoints[0]).toBe('page/handler');
      ...
      expect(jsx.actionEndpoints[0]).toBe('page/handler');
    });
  });
  ...
  describe('variable name assignment determines endpoint label (const pattern)', () => {
    ...
    it('converts PascalCase variable names to kebab-case', () => {
      const code = `const FetchUserData = $action(() => null)`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('users/fetch-user-data');
    });

    it('converts camelCase variable names to kebab-case', () => {
      const code = `const createNewItem = $action(() => null)`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('items/create-new-item');
    });
  ...
  describe('property assignment path determines endpoint label (nested object pattern)', () => {
    it('uses property name as the endpoint label (converted by toKebabCase)', () => {
      const code = `const api = { getTodos: $action(() => []) }`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
        '/project/src/api.ts',
        '/project',
      ...
      expect(actionEndpoints[0]).toBe('api/api-get-todos');
    });

    it('uses nested property path (dots become hyphens via toKebabCase)', () => {
      const code = `const routes = { users: { getAll: $action(() => []) } }`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('routes/routes-users-get-all');
    });

    it('combines deeply nested property path segments', () => {
      const code = `const config = { api: { v2: { fetchData: $action(() => null) } } }`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('config/config-api-v2-fetch-data');
    });
  });

  describe('fallback to $action@line:col / $ws@line:col when no context', () => {
    it('falls back to $action@line:col when not assigned to a variable', () => {
      const code = `export default $action(() => null)`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toBe('handler/default');
    });

    it('falls back to line:col format for unresolvable positions', () => {
      // A action call as an argument to another function has no variable context
      // The label becomes "arg0" since it's the first argument in a call expression
      const code = `register($action(() => null))`;
      const { actionEndpoints } = processAndGetEndpoints(
        code,
      ...
      expect(actionEndpoints[0]).toMatch(/setup\//);
    });
