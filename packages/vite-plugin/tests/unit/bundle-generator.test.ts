...
function makeBackendEntry(overrides: Partial<BackendEntry> & { endpoint: string; file: string }): BackendEntry {
  return {
    imports: [],
    fnJs: '(args) => args',
...
function makeWsEntry(overrides: Partial<WebSocketEntry> & { endpoint: string; file: string }): WebSocketEntry {
  return {
    imports: [],
...
describe('Bundle Generator Output Structure', () => {
  describe('Backend entries produce Hono POST route for /__server-build/*', () => {
    it('should produce a POST route for /__server-build/* when backend entries exist', () => {
      const registry = new Registry<BackendEntry>();
      const entry = makeBackendEntry({
  ...
  describe('Websocket entries produce Bun.serve config with fetch and websocket block', () => {
    it('should produce Bun.serve with fetch and websocket lifecycle handlers', () => {
      const registry = new Registry<BackendEntry>();
  ...
  describe('Null return when no backend, websocket, or server entries exist', () => {
