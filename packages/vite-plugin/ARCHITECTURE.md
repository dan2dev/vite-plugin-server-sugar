...
This document explains how `vite-plugin-server-sugar` works internally: the
...
{
  "compilerOptions": {
    "types": ["vite-plugin-server-sugar/server", "vite-plugin-server-sugar/ws"]
  }
}
```
...
`// Auto-generated by vite-plugin-server-sugar - do not edit` banner, that
