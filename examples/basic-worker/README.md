...
Create the production D1 database once and copy the returned UUID:

```bash
npx wrangler d1 create basic-worker-todos
```

Then expose that UUID while deploying:

```bash
CLOUDFLARE_D1_DATABASE_ID=<uuid> npm run deploy
```

The deploy script builds with that database ID, applies pending migrations to
the remote database, deploys each independent Worker, and deploys the gateway
